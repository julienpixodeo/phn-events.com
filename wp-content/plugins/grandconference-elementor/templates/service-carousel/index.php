<?php
	...
	if(!empty($slides))
	{
		...
		$count = 1;
?>
<div class="service-carousel-wrapper <?php echo esc_attr($settings['service_style']); ?>">
							...
							if(!empty($slide['slide_link']['url']))
								{
									$target = $slide['slide_link']['is_external'] ? 'target="_blank"' : '';
							?>
								<a href="<?php echo esc_url($slide['slide_link']['url']); ?>" class="service-carousel-button button"><?php echo esc_html($slide['slide_link_title']); ?></a>
							<?php
								}
							?>
						</div>
