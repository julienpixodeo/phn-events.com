...
?>
		...
		?>
			...
			<?php
				...
				{
					$course_price = grandconference_get_course_price_html($post_ID);
			?>
			...
			<div class="card-info">
				...
				<div class="tooltip-templates">
				    <div id="tooltip_content_<?php echo esc_attr($post_ID); ?>" class="course-tooltip-content">
					    <h5><?php esc_html_e("What you'll learn", 'grandconference-elementor' ); ?></h5>
				...
				?>
					...
					<div class="card-rating-total">
						...
						<?php
							if($total > 1)
							{
								echo esc_html_e('ratings', 'grandconference-elementor' );
							}
							else
							{
								echo esc_html_e('rating', 'grandconference-elementor' );	
							}
						?>)
					</div>
		    	<?php
				...
				<?php
					...
					{
				?>
				<div class="card-excerpt"><?php echo grandconference_limit_get_excerpt(strip_tags(get_the_excerpt()), $settings['excerpt_length']['size'], '...'); ?></div>
				<?php
					}
				?>
				...
				<?php
					$meta_class = '';
					if(function_exists('grandconference_get_course_curriculum_number') OR $settings['show_lesson'] != 'yes' OR $settings['show_student'] != 'yes')	
					{
						$meta_class = 'empty';
					}
				?>
				<div class="card-meta-wrapper <?php echo esc_attr($meta_class); ?>">
					<?php
					if(function_exists('grandconference_get_course_curriculum_number') && $settings['show_lesson'] == 'yes')
					{
						$course_lessons = grandconference_get_course_curriculum_number($post_ID);
					...
					<div class="card-meta">
						...
						<span class="card__lesson">
							...
							<?php
								if($course_lessons > 1)
								{
									echo esc_html_e('lessons', 'grandconference-elementor' );
								}
								else
								{
									echo esc_html_e('lesson', 'grandconference-elementor' );
								}
							?>
						</span>
					</div>
					...
					<div class="card-meta">
						...
						<span class="card__student">
							...
							<?php
								...
								{
									echo esc_html_e('students', 'grandconference-elementor' );
								}
								else
								{
									echo esc_html_e('student', 'grandconference-elementor' );
								}
							?>
						</span>
					</div>
...
<?php
	if($settings['show_pagination'] == 'yes')
	{
		...
		if($wp_query->max_num_pages > 1)
	    {
	    	...
	    	{
	    	    grandconference_pagination($wp_query->max_num_pages, 4, 'course-posts-grid' );
	    	}
	    	else
	    	{
?>
