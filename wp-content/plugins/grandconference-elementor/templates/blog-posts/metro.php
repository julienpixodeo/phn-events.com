...
<?php
	...
	{
		$blog_featured_image_size = 'grandconference-blog';
	}
?>
<?php
	if($counter == 2)
			    ...
			    <div class="post-featured-image-hover <?php if(!empty($tg_enable_lazy_loading)) { ?>lazy<?php } ?>">
					...
					?>
			     	...
			     	<?php 
				     	...
				     	{
				     ?>
				     <img <?php echo grandconference_get_blank_img_attr(); ?> <?php echo esc_attr($return_attr['source']); ?>="<?php echo esc_url($blog_featured_img_url); ?>" class="lazy_masonry smoove" alt="<?php echo esc_attr($blog_featured_img_alt); ?>"/>
				     <?php
					     }
					?>
