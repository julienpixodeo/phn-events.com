...
<div <?php post_class(array('blog-posts-'.$settings['layout'])); ?>>
	...
	<div class="post-wrapper <?php echo esc_attr($smoove_class); ?> <?php echo esc_attr($animation_class); ?>" data-delay="<?php echo intval($queue*100); ?>">
			    ...
			    <div class="post-featured-image-hover <?php if(!empty($tg_enable_lazy_loading)) { ?>lazy<?php } ?>">
					...
					?>
			     	...
			     	<?php 
				     	...
				     	{
				     ?>
				     <img <?php echo grandconference_get_blank_img_attr(); ?> <?php echo esc_attr($return_attr['source']); ?>="<?php echo esc_url($blog_featured_img_url); ?>" class="lazy_masonry smoove <?php echo esc_attr($animation_class); ?>" alt="<?php echo esc_attr($blog_featured_img_alt); ?>"/>
				     <?php
					     }
					?>
