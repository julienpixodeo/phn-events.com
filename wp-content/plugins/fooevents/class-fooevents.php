		...
		if ( is_admin() && function_exists( 'has_block' ) && function_exists( 'wc_get_page_id' ) ) {
			...
			if ( $checkout_page_id && has_block( 'woocommerce/checkout', $checkout_page_id ) ) {
				// Add an action to admin_notices to display a dashboard message
					echo '<div class="notice notice-error"><p>' . wp_kses_post( '<b>Important:</b> FooEvents does not currently support the new WooCommerce Checkout block which is currently being used on the WooCommerce Checkout page. Please switch to the classic shortcode-based WooCommerce Checkout experience to use FooEvents functionality on the Checkout screen. <a href="https://help.fooevents.com/docs/frequently-asked-questions/installation-setup/does-fooevents-work-with-the-woocommerce-checkout-and-cart-blocks/" target="_BLANK">Learn more.</a>', 'woocommerce-events' ) . '</p></div>';
			}
		}
...
if ( is_plugin_active( 'automatewoo/automatewoo.php' ) || is_plugin_active_for_network( 'automatewoo/automatewoo.php' ) ) {
	...
	require_once dirname( __FILE__ ) . '/classes/automatewoo/fooevents-automatewoo.php';
