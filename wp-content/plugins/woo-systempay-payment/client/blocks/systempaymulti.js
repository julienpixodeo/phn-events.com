...
const PAYMENT_METHOD_NAME = 'systempaymulti';
var systempay_data = getSystempayServerData(PAYMENT_METHOD_NAME);

const Content = () => {
    var fields = <ul dangerouslySetInnerHTML={{__html: systempay_data?.payment_fields}} />;
    jQuery('.wc-block-components-checkout-place-order-button').on('click', systempay_get_selected_option(['systempaymulti_option', 'systempaymulti_card_type']));

    return (
        <div>
            { systempay_data?.description }
            { fields }
        </div>
    );
};
