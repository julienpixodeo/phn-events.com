...
jQuery(function() {
    if ((typeof SYSTEMPAY_HIDE_ADD_PAYMENT_LINK !== "undefined") && (SYSTEMPAY_HIDE_ADD_PAYMENT_LINK != true)) {
       jQuery(".woocommerce-MyAccount-content a.button:not(.delete)").hide();
    }

    if (typeof IDENTIFIER_FORM_TOKEN !== "undefined") {
        systempayUpdateFormToken(true);
    }
...
var SYSTEMPAY_EXPIRY_ERRORS = ['PSP_108', 'PSP_136', 'PSP_649'];

var systempayInitRestEvents = function(KR) {
    KR.onError(function(e) {
        jQuery('#systempay_rest_processing').css('display', 'none');
        ...
        if (typeof window.SYSTEMPAY_BUTTON_TEXT != 'undefined') {
        ...
        if (SYSTEMPAY_EXPIRY_ERRORS.indexOf(e.errorCode) >= 0) {
            msg += ' <a href="#" onclick="window.location.reload(); return false;">'
                + systempayTranslate('RELOAD_LINK') + '</a>';
        }
...
var systempayDrawRestPaymentFields = function(formToken, first) {
    ...
    var sfStyle = (SYSTEMPAY_HIDE_SINGLE_BUTTON == true) ? 'style="width: 100%;"' : '';
    var fields = '<div class="' + SYSTEMPAY_KR_MODE + '" ' + SYSTEMPAY_SINGLE_PAYMENT_BUTTON_MODE + ' ' + SYSTEMPAY_SINGLE_PAYMENT_BUTTON_MODE + ' ' + SYSTEMPAY_SF_EXTENDED_MODE + ' ' + SYSTEMPAY_POPIN_ATTR + ' ' + sfStyle + ' > ' + cardsForm + ' <div class="kr-form-error"></div> <div id="systempay_rest_processing" class="kr-field processing" style="display: none; border: none; z-index: -1;"> <div style="background-image: url(' + SYSTEMPAY_IMG_URL + '); margin: 0 auto; display: block; height: 35px; background-position: center; background-repeat: no-repeat; background-size: 35px;"></div></div></div>';

    if (jQuery("#systempaystd_rest_wrapper").length) {
        jQuery("#systempaystd_rest_wrapper").html(fields);
    } else {
        jQuery("#systempaywcssubscription_rest_wrapper").html(fields);
    }

    KR.renderElements();

    var systempayFormConfig = { language: SYSTEMPAY_LANGUAGE, formToken: formToken };
    if (SYSTEMPAY_HIDE_SINGLE_BUTTON == true) {
        systempayFormConfig['form'] = { smartform: { singlePaymentButton: { visibility: false }}};
    }

    if (SYSTEMPAY_SF_COMPACT_MODE == true) {
        systempayFormConfig['cardForm'] = { layout: 'compact' };
        systempayFormConfig['smartForm'] = { layout: 'compact' };
    }

    if (SYSTEMPAY_GROUPING_THRESHOLD != 'false') {
        systempayFormConfig['smartForm']['groupingThreshold'] = SYSTEMPAY_GROUPING_THRESHOLD;
    }

    setTimeout(function () {
        KR.setFormConfig(systempayFormConfig).then(function(v) {
            if (first) {
