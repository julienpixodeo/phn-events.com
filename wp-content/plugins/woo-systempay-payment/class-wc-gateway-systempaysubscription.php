...
class WC_Gateway_SystempaySubscription extends WC_Gateway_SystempayStd
{
    ...
    public function __construct()
    {
        ...
        $this->method_description = sprintf(__('Subscriptions managed by %s gateway', 'woo-systempay-payment'), self::GATEWAY_NAME)
            . ' <b>('. __('Deprecated', 'woo-systempay-payment') . ')</b>';
    ...
    public function init_form_fields()
    {
        ...
        $this->form_fields['subscriptions'] = array(
            'custom_attributes' => array(
                'onchange' => 'systempayShowSubscriptionsWarningMessage()'
            ),
    ...
    public function cancel_online_subscription($subscription_id, $order_id)
    {
        ...
        $cust_id = self::get_order_property($order, 'user_id');
        $saved_identifier = $this->get_cust_identifier($cust_id);
        $subscriptionId = (SystempayTools::is_hpos_enabled()) ? $order->get_meta('Subscription ID') : get_post_meta($order_id, 'Subscription ID', true);
    ...
    public function update_online_subscription($subscription_id, $order_id)
    {
        ...
        $subscriptionId = (SystempayTools::is_hpos_enabled()) ? $order->get_meta('Subscription ID', true) : get_post_meta($order_id, 'Subscription ID', true);
    ...
    public function systempay_admin_head_script()
    {
        ...
        <script type="text/javascript">
            //<!--
            function systempayShowSubscriptionsWarningMessage() {
                var subscriptions = jQuery('#<?php echo esc_attr($this->get_field_key('subscriptions')); ?> option:selected').val();
                if ((<?php echo SystempayTools::is_plugin_not_active('woocommerce-subscriptions/woocommerce-subscriptions.php'); ?>) && (subscriptions === 'wc-subscriptions') &&
                    ! confirm('<?php echo sprintf(__('Warning! %s plugin must be installed and activated for the subscription payment method to work.', 'woo-systempay-payment'), 'WooCommerce Subscriptions')?>')) {
                    systempayResetSubscriptionsField();
                } else if ((<?php echo SystempayTools::is_plugin_not_active('subscriptio/subscriptio.php'); ?>) && (subscriptions === 'subscriptio') &&
                    ! confirm('<?php echo sprintf(__('Warning! %s plugin must be installed and activated for the subscription payment method to work.', 'woo-systempay-payment'), 'Subscriptio')?>')) {
                    systempayResetSubscriptionsField();
                } else if ((subscriptions === 'custom') &&
                    ! confirm('<?php echo __('Warning! You have to implement a subscriptions adapter for our plugin.', 'woo-systempay-payment')?>')) {
                    systempayResetSubscriptionsField();
                }
            }

            function systempayResetSubscriptionsField() {
...
<?php
    ...
    public function admin_options()
    {
        if ($this->get_option('subscriptions') === 'wc-subscriptions'
            && SystempayTools::is_plugin_not_active('woocommerce-subscriptions/woocommerce-subscriptions.php') === 'true') {
    ...
    public function process_admin_options()
    {
        ...
        $wcs_subscription_settings = get_option('woocommerce_systempaywcssubscription_settings', null);
        ...
        WC_Admin_Settings::add_error(sprintf(__('This method cannot be enabled. You have to disable "%s" method first.', 'woo-systempay-payment'),
            self::GATEWAY_NAME . ' - ' . __('Subscription payment with WooCommerce Subscriptions', 'woo-systempay-payment')));
