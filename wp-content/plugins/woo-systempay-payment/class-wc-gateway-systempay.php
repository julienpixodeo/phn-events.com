...
class WC_Gateway_Systempay extends WC_Payment_Gateway
{
    ...
    /**
     * @var SystempayRequest
     */
    protected $systempay_request;
    ...
    public function __construct()
    {
        ...
        add_action('woocommerce_before_thankyou', array($this, 'systempay_thankyou'), 10, 4);
        ...
        add_action('woocommerce_order_status_changed', array($this, 'systempay_delete_order_with_status_failed'), 10, 1);

        // Filter to allow customer saved payment means management.
        add_filter('woocommerce_saved_payment_methods_list', array($this, 'systempay_get_account_saved_payment_methods_list'), 10, 2);
    }
    ...
    public function systempay_admin_head_script()
    {
        ...
        //<!--
            jQuery(function() {
                ...
                systempayUpdateShippingOptionsDisplay();
            });
    ...
    public function init_form_fields()
    {
        ...
        $this->form_fields = array(
            ...
            'embedded_fields_keys_settings' => array(
                'title' => '',
                'type' => 'title',
                'description' => sprintf(__('Configure this section only if you are using embedded payment fields or Smartform modes.', 'woo-systempay-payment'), self::BACKOFFICE_NAME)
            ),
            ...
            'delete_order_on_failure' => array(
                'title' => __('Delete order on failure', 'woo-systempay-payment'),
                'label' => __('Enable / disable', 'woo-systempay-payment'),
                'type' => 'checkbox',
                'default' => 'no',
                'description' => __('If enabled, the order will be deleted on WooCommerce when the payment is declined.', 'woo-systempay-payment')
            ),
        ...
        $columns['method_title'] = array(
            'title' => __('Method title', 'woo-systempay-payment'),
            'width' => '210px'
        );

        $columns['type'] = array(
            'title' => __('Type', 'woo-systempay-payment'),
            'width' => '130px'
        );

        $columns['speed'] = array(
            'title' => __('Rapidity', 'woo-systempay-payment'),
            'width' => '75px',
        );

        $columns['delay'] = array(
            'title' => __('Delay', 'woo-systempay-payment'),
            'width' => '90px',
        );

        $this->form_fields['shipping_options'] = array(
            'title' => __('Shipping options', 'woo-systempay-payment'),
            'type' => 'shipping_table',
            'columns' => $columns,
            'description' => $descr
        );

        if (isset($systempay_plugin_features['qualif']) && $systempay_plugin_features['qualif']) {
        ...
        if (isset($systempay_plugin_features['shatwo']) && $systempay_plugin_features['shatwo']) {
    ...
    public function generate_shipping_table_html($key, $data)
    {
        ...
        $shipping_types = array(
            'PACKAGE_DELIVERY_COMPANY' => __('Delivery company', 'woo-systempay-payment'),
            'RECLAIM_IN_SHOP' => __('Reclaim in shop', 'woo-systempay-payment'),
            'RELAY_POINT' => __('Relay point', 'woo-systempay-payment'),
            'RECLAIM_IN_STATION' => __('Reclaim in station', 'woo-systempay-payment')
        );

        $shipping_rapidities = array(
            'STANDARD' => __('Standard', 'woo-systempay-payment'),
            'EXPRESS' => __('Express', 'woo-systempay-payment'),
            'PRIORITY' => __('Priority', 'woo-systempay-payment')
        );

        $shipping_delays = array(
            'INFERIOR_EQUALS' => __('<= 1 hour', 'woo-systempay-payment'),
            'SUPERIOR' => __('> 1 hour', 'woo-systempay-payment'),
            'IMMEDIATE' => __('Immediate', 'woo-systempay-payment'),
            'ALWAYS' =>  __('24/7', 'woo-systempay-payment'),
        );
        ...
        foreach ($options as $code => $option) {
            ...
            $html .= '<td style="padding: 5px;"><select class="systempay_list_speed" id="' . $field_name . '_' . $code . '_speed"
                      name="' . $field_name . '[' . $code . '][speed]"
                      value="' . (isset($option['speed']) ? $option['speed'] : '') . '"
                      onchange="javascript:systempayUpdateShippingOptionsDisplay(\'' . $field_name . '_' . $code . '_speed\')">';
    ...
    public function systempay_notify_response()
    {
        ...
        if ($session_id = $systempay_response->getExtInfo('session_id')) {
            set_transient('systempay_session_id', $session_id);
        }
     ...
     **/
    public function systempay_manage_notify_response($systempay_response, $from_server_rest = false, $update_on_failure = true, $server_rest_abandoned_payment = false)
    {
        global $woocommerce, $systempay_plugin_features;
        ...
        if ($server_rest_abandoned_payment && SystempayTools::is_embedded_payment(false) && empty($systempay_response->getTransStatus())) {
        ...
        if (! $order && ($this->get_general_option('delete_order_on_failure') == "yes")
            && ! self::is_successful_action($systempay_response)) {
        ...
        if (! $order || ! self::get_order_property($order, 'id') || ($key && (self::get_order_property($order, 'order_key') !== $key))) {
            if ($systempay_response->getExtInfo('from_account')) {
                if ($systempay_response->get('identifier') || ($systempay_response->get('identifier_status') == 'CREATED')) {
                ...
                $this->systempay_redirect(wc_get_account_endpoint_url('payment-methods'));
            }
        ...
        if (SystempayRestTools::checkResponse($_POST)) {
            $systempay_method = $systempay_response->getExtInfo('wcs_scheduled') ? new WC_Gateway_SystempayWcsSubscription : new WC_Gateway_SystempayStd();
            if (method_exists($order, 'set_payment_method')) {
                $order->set_payment_method($systempay_method);
            } else {
                $order->payment_method = $systempay_method->id;
                $order->payment_method_title = $systempay_method->get_title();
            }
        ...
        if ($this->is_new_order($order, $systempay_response->get('trans_id'), $systempay_response->get('sequence_number'))) {
            // Clear transients.
            delete_transient('systempay_token_data_' . $systempay_response->getExtInfo('session_id'));
            delete_transient('systempay_token_data_identifier_' . $systempay_response->getExtInfo('session_id'));
            delete_transient('systempay_token_' . $systempay_response->getExtInfo('session_id'));
            delete_transient('systempay_token_identifier_' . $systempay_response->getExtInfo('session_id'));

            // Add order note.
            self::systempay_add_order_note($systempay_response, $order);

            self::systempay_update_order_meta($systempay_response, $order);
            ...
            $wcs_scheduled = ($method === 'systempaywcssubscription');
            ...
            if ($systempay_response->get('recurrence_number') && $subscriptions_handler) {
                ...
                || (($systempay_response->get('identifier_status') === 'CREATED') && $wcs_scheduled)
                ...
                if ($subsc_id = $systempay_response->getExtInfo('subsc_id')) {
                    $subsc_redirect_url = isset($subscriptions_handler) ? $subscriptions_handler->get_view_order_url($subsc_id) : SystempayTools::get_view_order_url($subsc_id);
                }
            ...
            if ($from_server && ($order_status === 'on-hold')) {
                ...
                $this->log('IPN URL PROCESS END');
                die();
            } elseif (self::is_successful_action($systempay_response) && key_exists($order_status, self::get_success_order_statuses(false, $subscriptions_handler))) {
            ...
            } elseif (! self::is_successful_action($systempay_response) && ($order_status === 'failed' || $order_status === 'cancelled')) {
                ...
                $this->log("Error! Invalid payment result received for already saved order #$order_id. Payment result : {$systempay_response->getTransStatus()}, Order status : {$order_status}.");
    ...
    {
        if ($order->has_status(apply_filters('woocommerce_systempay_valid_order_statuses', array('pending', 'checkout-draft'), $order))) {
            return true;
        }
        ...
        if (function_exists('wcs_get_subscription')) {
            ...
            if ($subscription && strpos(self::get_order_property($order, 'payment_method'), 'systempay') === false) {
                return true;
            }
        }

        if (SystempayTools::is_hpos_enabled()) {
    ...
    protected function clear_notices()
    {
        ...
        delete_transient('systempay_notices_' . wp_get_session_token());
    ...
    protected function add_notice($msg, $type = 'success')
    {
        ...
        if (function_exists('wc_add_notice')) {
            ...
            if (SystempayTools::has_checkout_block() && ($type == 'error')) {
                $type = 'notice';
            }
    ...
    protected function systempay_redirect($url, $iframe = false)
    {
        ...
        if (($session_id = get_transient('systempay_session_id')) && ! empty($wc_notices)) {
            set_transient('systempay_notices_' . $session_id, json_encode($wc_notices));
            delete_transient('systempay_session_id');
        }
    ...
    public static function systempay_update_order_meta($systempay_response, $order)
    {
        if (SystempayTools::is_hpos_enabled()) {
            ...
            $order->update_meta_data('Transaction ID', $systempay_response->get('trans_id'));
            $order->update_meta_data('Card number', $systempay_response->get('card_number'));
            $order->update_meta_data('Means of payment', $systempay_response->get('card_brand'));
            $order->update_meta_data('Sequence number', $systempay_response->get('sequence_number'));
            ...
            if ($authorized_amount = $systempay_response->get('authorized_amount')) {
                ...
                $order->update_meta_data('Authorized amount', self::display_amount($authorized_amount, $systempay_response->get('currency')));
            }
            ...
            $order_id = (int) $systempay_response->get('order_id');
            ...
            // Store transaction details.
            update_post_meta($order_id, 'Transaction ID', $systempay_response->get('trans_id'));
            update_post_meta($order_id, 'Card number', $systempay_response->get('card_number'));
            update_post_meta($order_id, 'Means of payment', $systempay_response->get('card_brand'));
            update_post_meta($order_id, 'Sequence number', $systempay_response->get('sequence_number'));

            // Store authorized amount.
            if ($authorized_amount = $systempay_response->get('authorized_amount')) {
                delete_post_meta($order_id, 'Authorized amount');
                update_post_meta($order_id, 'Authorized amount', self::display_amount($authorized_amount, $systempay_response->get('currency')));
            }
    ...
    public static function restore_wc_notices() {
        ...
        if (WC()->session && empty(WC()->session->get('wc_notices', array())) && get_transient('systempay_notices_' . wp_get_session_token())) {
            wc_set_notices(json_decode(get_transient('systempay_notices_' . wp_get_session_token()), true));
            delete_transient('systempay_notices_' . wp_get_session_token());
        }
    }
    ...
    public function systempay_thankyou($order_id)
    {
        $order = wc_get_order($order_id);
    ...
    public static function is_successful_action($systempay_response)
    {
        ...
        if (! $systempay_response->getExtInfo('is_rest') && $systempay_response->get('identifier') && (
    ...
    private function systempay_save_identifier($order, $systempay_response, $force_update = false)
    {
        ...
        if ($systempay_response->get('identifier') && ($force_update || (
            ...
            $subsc_id = $systempay_response->getExtInfo('subsc_id') ? $systempay_response->getExtInfo('subsc_id') : $systempay_response->get('order_id');
            $this->log("Saving identifier for order #$subsc_id in meta data.");

            if (SystempayTools::is_hpos_enabled()) {
                $subscription = wc_get_order($subsc_id);
                $subscription->update_meta_data('systempay_token', $systempay_response->get('identifier'));
                $subscription->save();
            } else {
                update_post_meta($subsc_id, 'systempay_token', $systempay_response->get('identifier'));
            }
            ...
            if (function_exists('wcs_get_subscriptions_for_order')) {
                ...
                foreach (wcs_get_subscriptions_for_order($order) as $subscription) {
                    if (SystempayTools::is_hpos_enabled()) {
                        ...
                        update_post_meta($subscription->get_id(), 'systempay_token', $systempay_response->get('identifier'));
                    }
                }

                if ($systempay_response->getExtInfo('update_identifier_all')) {
                    foreach (wcs_get_subscriptions_for_order($order) as $subscription) {
                        if (SystempayTools::is_hpos_enabled()) {
                            $subscription->update_meta_data('systempay_token', $systempay_response->get('identifier'));
                            $subscription->save();
            ...
            if ($systempay_response->getExtInfo('is_customer_wallet')) {
                return;
            }
    ...
    private function systempay_save_recurrence($order, $systempay_response, $force_update = false)
    {
        ...
        if ($systempay_response->get('subscription') && ($force_update || $systempay_response->get('recurrence_status') === 'CREATED')) {
            ...
            if (SystempayTools::is_hpos_enabled()) {
                ...
                // Store subscription details.
                $order->update_meta_data('Subscription ID', $systempay_response->get('subscription'));
                $order->update_meta_data('Subscription amount', self::display_amount($systempay_response->get('sub_amount'), $currency_code));
                $order->update_meta_data('Effect date', preg_replace('#^(\d{4})(\d{2})(\d{2})$#', '\1-\2-\3', $systempay_response->get('sub_effect_date')));

                if ($systempay_response->get('sub_init_amount')) {
                    $order->update_meta_data('Initial amount', self::display_amount($systempay_response->get('sub_init_amount'), $currency_code));
                    $order->update_meta_data('Initial amount count', $systempay_response->get('sub_init_amount_number'));
                }
    ...
    public function systempay_delete_saved_card()
    {
        ...
        if (isset($_POST['token']) && $_POST['token']) {
            $sub_modules = array('systempaystd', 'systempaysubscription', 'systempaywcssubscription');
    ...
    function systempay_delete_order_with_status_failed($order_id)
    {
        // Get an instance of the order object.
        $order = wc_get_order($order_id);

        if (($this->get_general_option('delete_order_on_failure') == 'yes')
            && (strpos(self::get_order_property($order, 'payment_method'), 'systempay') === 0)
    ...
    function systempay_get_account_saved_payment_methods_list($list, $cust_id)
    {
        if (! is_add_payment_method_page()) {
            return $list;
        }

        $sub_modules = array('systempaysubscription');

        $systempay_wcssubscription = new WC_Gateway_SystempayWcsSubscription();
        if ($systempay_wcssubscription->is_available() && $systempay_wcssubscription->use_wallet($cust_id)) {
            return $list;
        } else {
            $sub_modules[] = 'systempaywcssubscription';
        }

        $systempay_std = new WC_Gateway_SystempayStd();
        if ($systempay_std->use_wallet($cust_id)) {
            return $list;
        } else {
            $sub_modules[] = 'systempaystd';
        }
        ...
        foreach ($sub_modules as $id) {
            ...
            if ($saved_masked_pan) {
                ...
                $delete_url = add_query_arg(array(
                    'wc-api' => 'WC_Gateway_Systempay_Delete_Saved_Card',
