...
<?php
    ...
    public function get_payment_fields($options = array())
    {
        ...
        if (count($options) == 1) {
            ...
            $html .= '<span style="font-weight: bold;">' . __('Your payment option', 'woo-systempay-payment') . '</span>';
            ...
            $first = true;
            $html .= '<span style="font-weight: bold;">' . __('Choose your payment option', 'woo-systempay-payment') . '</span>';
            foreach ($options as $key => $option) {
                $html .= '<li style="list-style-type: none;">
                        <input class="radio" type="radio"' . ($first ? ' checked="checked"' : '') . ' id="systempayfranfinance_option_' . $key . '" value="' . $key . '" name="systempayfranfinance_option">
    ...
    public function process_payment($order_id)
    {
        ...
        $options = $this->get_available_options();
        $option_id = isset($_POST['systempayfranfinance_option']) ? $_POST['systempayfranfinance_option'] : $_COOKIE['systempayfranfinance_option'];
