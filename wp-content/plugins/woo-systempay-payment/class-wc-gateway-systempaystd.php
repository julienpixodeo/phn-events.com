...
class WC_Gateway_SystempayStd extends WC_Gateway_Systempay
{
    ...
    public function __construct()
    {
        ...
        add_filter('woocommerce_available_payment_gateways', array($this, 'remove_systempay_payment_method'));
    }
    ...
    public function systempay_rest_head_script()
    {
        ...
        if ($this->is_embedded_payment(false) && $this->is_available()) {
            ...
            ?>
                ...
                <style>
                    .systempay_rest_wrapper button.kr-popin-button {
    ...
    public function init_form_fields()
    {
        ...
        $this->form_fields['use_customer_wallet'] = array(
            'title' => __('Use buyer wallet to manage tokens', 'woo-systempay-payment'),
            'type' => 'select',
            'default' => '0',
            'options' => array(
                '1' => __('Yes', 'woo-systempay-payment'),
    ...
    protected function rest_payment_fields_view($use_identifier)
    {
        ...
        $html .= '<div id="' . $this->id . '_rest_wrapper" class="systempay_rest_wrapper"></div>';
        $html .= '<script type="text/javascript">';
        ...
        $jsVars .= 'window.SYSTEMPAY_DELETE_CARD_URL = "'. add_query_arg('wc-api', 'WC_Gateway_Systempay_Delete_Saved_Card', home_url('/')). '";';
        ...
        $jsVars .= 'window.SYSTEMPAY_HIDE_ADD_PAYMENT_LINK = new Boolean(' . ($show_add_payment_method_link && $this->use_wallet()). ');';
            ...
            jQuery(document).ready(function() {
                ...
                jQuery("#add_payment_method").submit(function(event) {
                    ...
                    jQuery("div.blockUI.blockOverlay").hide();
                     if (SYSTEMPAY_HIDE_SINGLE_BUTTON) {
                         KR.openSelectedPaymentMethod();
                     } else {
                        KR.submit();
                     }
                ...
                jQuery("#place_order").click(function(event) {
                    ...
                    if ((! useIdentifier) && (! popin)  && (! SYSTEMPAY_HIDE_SINGLE_BUTTON) && (! ' . $hide_smart_button_popin . ')) {
                        if (formIsValidated) {
    ...
    protected function get_smartform_cards()
    {
        ...
        if ($this->systempay_other_payment_methods && $this->systempay_other_payment_methods->is_available_ignoring_regroup()) {
    ...
    private function get_temporary_form_token($use_identifier = false)
    {
        ...
        if (($woocommerce->cart->total <= 0) && ($this->id === 'systempaywcssubscription')) {
        ...
        if ($this->id === 'systempaywcssubscription' && $this->use_wallet($cust_id)) {
        ...
        if ($token) {
            ...
            set_transient('systempay_token_' . ($use_identifier ? 'identifier_' : '') . wp_get_session_token(), $token);
        }
    ...
    protected function get_form_token($order, $use_identifier = false)
    {
        ...
        if ($this->get_escaped_var($this->systempay_request, 'ext_info_wcs_scheduled')) {
            $params['metadata']['wcs_scheduled'] = true;
        }

        if ($this->get_escaped_var($this->systempay_request, 'ext_info_subsc_id')) {
            $params['metadata']['subsc_id'] = $this->get_escaped_var($this->systempay_request, 'ext_info_subsc_id');
        }

        if ($this->get_escaped_var($this->systempay_request, 'ext_info_update_identifier_all')) {
        ...
        if ($this->id === 'systempaywcssubscription' && $this->use_wallet($cust_id)) {
            $params['formAction'] = ($this->systempay_request->get('amount') == 0) ? 'REGISTER' : 'REGISTER_PAY';
            $params['enableCustomerTokens'] = true;
            $params['metadata']['is_customer_wallet'] = true;
        } elseif ($this->use_wallet($cust_id)) {
            $params['formAction'] = ($this->systempay_request->get('amount') == 0) ? 'REGISTER' :  'CUSTOMER_WALLET';
            $params['metadata']['is_customer_wallet'] = true;
        } elseif ($use_identifier) {
            $saved_identifier = $this->get_cust_identifier($cust_id);
            $params['paymentMethodToken'] = $saved_identifier;
    ...
    public function process_payment($order_id)
    {
        ...
        $order = new WC_Order($order_id);
        $force_redir = isset($_POST['systempay_force_redir']) || (isset($_COOKIE[$this->id. '_force_redir']) && ! empty($_COOKIE[$this->id. '_force_redir']));
    ...
    protected function systempay_fill_request($order)
    {
        ...
        $currency = SystempayApi::findCurrencyByAlphaCode($order_currency);
        if ($currency == null) {
    ...
    public function systempay_manage_rest_notify_response($from_server_rest = false)
    {
        ...
        parent::systempay_manage_notify_response($response, $from_server_rest, $update_on_failure, $server_rest_abandoned_payment);
    }
    ...
    public function process_refund($order_id, $amount = null, $reason = '')
    {
        ...
        systempay_launch_online_refund($order, $amount, $order->get_currency());
        ...
        $message = sprintf(__('Online Refund of %1$s - Reason: %2$s.', 'woo-systempay-payment'), $amount . $order->get_currency(), $reason);
     ...
     **/
    public function remove_systempay_payment_method($available_gateways)
    {
        global $wp;
        ...
        if (isset($available_gateways[$this->id])) {
            ...
            } elseif (isset($available_gateways['systempaywcssubscription'])) {
                unset($available_gateways['systempaywcssubscription']);
            }
        } else {
            ...
            foreach ($gateways as $gateway) {
                if (($gateway->id === $this->id) && $gateway->is_available() && $this->use_wallet()) {
                    ...
                    if (isset($available_gateways['systempaywcssubscription'])) {
                        unset($available_gateways['systempaywcssubscription']);
                    }
    ...
    public function is_embedded_payment($only_smartform = true)
    {
        ...
        return SystempayTools::is_embedded_payment($only_smartform);
    }
}
