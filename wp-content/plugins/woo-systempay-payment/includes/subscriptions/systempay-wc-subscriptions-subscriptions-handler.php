...
class Systempay_WC_Subscriptions_Subscriptions_Handler implements Systempay_Subscriptions_Handler_Interface
{
    ...
    public function cart_contains_subscription($cart = null)
    {
        ...
        if ($order = WC_Gateway_SystempayStd::order_created_from_bo()) {
    ...
    public function subscription_info($order)
    {
        ...
        if ($is_payment_change) {
            ...
            if (($old_payment_method === 'systempaysubscription') && $subscription_meta) {
                return false;
            }
    ...
    public function process_subscription($order, $response)
    {
        ...
        if (SystempayTools::is_hpos_enabled()) {
            ...
            $subscription->update_meta_data('Subscription amount', WC_Gateway_Systempay::display_amount($response->get('sub_amount'), $response->get('sub_currency')));
        ...
        if ($renewal_order->has_status('pending')) {
            try {
                ...
                if (SystempayTools::is_hpos_enabled()) {
                    ...
                    $renewal_order->update_meta_data('Subscription amount', WC_Gateway_Systempay::display_amount($response->get('amount'), $currency_code));
    ...
    private function get_renewal_order($subscription, $recurrence)
    {
        foreach ($subscription->get_related_orders('ids', 'renewal') as $renewal_order_id) {
            ...
            $recurrenceNum = (SystempayTools::is_hpos_enabled()) ? $order->get_meta('Recurrence number', true) : get_post_meta($renewal_order_id, 'Recurrence number', true);
