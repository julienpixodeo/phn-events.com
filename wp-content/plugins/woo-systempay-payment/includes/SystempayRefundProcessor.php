...
use Lyranetwork\Systempay\Sdk\Refund\Processor as RefundProcessor;

class SystempayRefundProcessor implements RefundProcessor
{
    protected $logger;
    ...
    {
        return new WP_Error(
            'systempay_error', sprintf($this->translate('There was a problem initiating an automatic refund. (ERROR %1$s)'), $errorCode),
            $message
            );
    }
    ...
    {
        $general_settings = get_option('woocommerce_systempay_settings', null);
        ...
        $this->logger->add('systempay', $message);
    }
    ...
    {
        return __($message, 'woo-systempay-payment');
    }
}
