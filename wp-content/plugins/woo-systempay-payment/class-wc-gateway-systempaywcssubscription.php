...
class WC_Gateway_SystempayWcsSubscription extends WC_Gateway_SystempayStd
{
    ...
    {
        $this->id = 'systempaywcssubscription';
        $this->icon = apply_filters('woocommerce_' . $this->id . '_icon', WC_SYSTEMPAY_PLUGIN_URL . 'assets/images/systempay.png');
        $this->has_fields = true;
        $this->method_title = self::GATEWAY_NAME . ' - ' . __('Subscription payment with WooCommerce Subscriptions', 'woo-systempay-payment');
        $this->method_description = __('Subscriptions managed by WooCommerce Subscriptions', 'woo-systempay-payment')
            . ' <b>(' . __('Recommended', 'woo-systempay-payment') . ')</b>';
        ...
        $this->subscriptions_handler = Systempay_Subscriptions_Loader::getInstance(self::SUBSCRIPTIONS_HANDLER);
        ...
        add_action('woocommerce_api_wc_gateway_' . $this->id . '_rest', array($this, 'systempay_rest_return_response'));
        ...
        add_action('woocommerce_api_wc_gateway_' . $this->id . '_form_token', array($this, 'systempay_refresh_form_token'));
    ...
    */
    ...
    {
        if (! SystempayTools::is_embedded_payment(true)) {
        ...
        $std_settings = get_option('woocommerce_systempaystd_settings', null);
                  ...
                  <ul>
                      <li style="list-style-type: none;">
                          <span>' .
                              ...
                              . ' (<a href="' . esc_url(wc_get_account_endpoint_url('payment-methods')) . '">' . __('manage your payment means', 'woo-systempay-payment') . '</a>).
                          </span>
                      </li>
                  </ul>
              </div>';
    ...
    protected function systempay_fill_request($order)
    {
        ...
        if ($saved_identifier && $is_identifier_active) {
            ...
            $action = ($this->systempay_request->get('amount') == 0) ? 'REGISTER_UPDATE' : 'REGISTER_UPDATE_PAY';
        } else {
            $action = ($this->systempay_request->get('amount') == 0) ? 'REGISTER' : 'REGISTER_PAY';
        }

        $this->systempay_request->set('page_action', $action);
        $this->systempay_request->addExtInfo('wcs_scheduled', true);

        if (isset($_POST['update_all_subscriptions_payment_method']) && $_POST['update_all_subscriptions_payment_method']) {
            $this->systempay_request->addExtInfo('update_identifier_all', true);
        }
    }
    ...
    {
        if (SystempayTools::is_plugin_not_active('woocommerce-subscriptions/woocommerce-subscriptions.php') === 'true') {
            echo '<div class="inline error"><p><strong>' . sprintf(__('Warning! %s plugin must be installed and activated for the subscription payment method to work.', 'woo-systempay-payment'), 'WooCommerce Subscriptions') . '</strong></p></div>';
        }
    ...
    public function process_subscription_payment($renewal_total, WC_Order $renewal_order)
    {
        ...
        if (! $key) {
            $error_msg = __('Private key is not configured. Subscription renewal cannot be processed.', 'woo-systempay-payment');
        ...
        if (! $saved_identifier) {
            ...
            $error_msg = __('Customer has no valid identifier. Subscription renewal cannot be processed.', 'woo-systempay-payment');
            $this->set_error($renewal_order, $subscription, $error_msg);
        ...
        if (! $payment_result) {
            $error_msg = sprintf(__('An error has occurred during the renewal of the subscription. Please consult the %s logs for more details.', 'woo-systempay-payment'), 'Systempay');
            $this->set_error($renewal_order, $subscription, $error_msg);
        ...
        $data = SystempayRestTools::convertRestResult($payment_result);
    ...
    public function update_renewal_order($renewal_order, $response)
    {
        ...
        try {
            $order_note = sprintf(_x('Processed renewal payment for order #%s.', 'used in order note', 'woo-systempay-payment'), $renewal_order_id);
            ...
            $order_note = sprintf(__("Error while processing order: %s", 'woo-systempay-payment'), $e->getMessage());
    ...
    protected function silent_payment($renewal_total, WC_Order $renewal_order, $saved_identifier)
    {
        ...
        $currency = SystempayApi::findCurrencyByAlphaCode($renewal_order->get_currency());

        $subscriptions = wcs_get_subscriptions_for_order($renewal_order, array('order_type' => array('renewal')));
        $subscription = reset($subscriptions); // Get first subscription.

        $this->systempay_fill_request($renewal_order);
        ...
        $validationMode = $this->get_escaped_var($this->systempay_request, 'validation_mode');
    ...
    {
        $saved_meta = $subscription->get_meta('systempay_token');
        ...
        $payment_meta[$this->id] = array(
            'post_meta' => array(
                'systempay_token' => array(
                    'value' => $saved_meta,
                    'label' => sprintf(__('%s token', 'woo-systempay-payment'), self::GATEWAY_NAME)
                )
            )
        );
    ...
    {
        $saved_identifier = $subscription->get_meta('systempay_token');
        if (! $saved_identifier) {
            $saved_identifier = $this->get_cust_identifier($cust_id);
        }
    ...
    protected function set_error($order, $subscription, $error_msg)
    {
        ...
        if (is_admin()) { // Show error message only if it's made on backend.
            set_transient('systempay_renewal_error_msg', $error_msg);
        }
    }
    ...
    public function process_admin_options()
    {
        ...
        $subscription_settings = get_option('woocommerce_systempaysubscription_settings', null);
        ...
        WC_Admin_Settings::add_error(sprintf(__('This method cannot be enabled. You have to disable "%s" method first.', 'woo-systempay-payment'),
            self::GATEWAY_NAME . ' - ' . __('Subscription payment', 'woo-systempay-payment')));

        $settings['enabled'] = 'no';
        update_option('woocommerce_' . $this->id . '_settings', $settings);
    }

    public function systempay_init()
    {
        parent::systempay_init();
